...
.color {
	background-color: #44A1D7 !important;
}
</style>
</head>
<body>
											...
											<c:if test="${not empty attachment }">
												<c:choose>
													<c:when test="${fileType =='pdf'}">
														<td>Pdf</td>
														<td><a
															href="http://rentmanager.princeglobalconsultants.com/birddogUploads/bird_pdf/${attachment.id}_${attachment.name}">${attachment.name}</a>
													</c:when>
													<c:otherwise>
														<td>Software</td>
														<td><a
															href="downloadAttachment?id=${attachment.id}&fileName=${attachment.name}">${attachment.name}</a></td>
													</c:otherwise>
												</c:choose>
											</c:if>
										</tr>
